...
**Date:** 2026-02-01  
**Status:** In Progress (building /agent surface)  
**Owner:** Tanveer  
...
- **Model routing:** Default to `openai/gpt-oss-20b` via `OPENROUTER_MODEL` env; keep swappable.
...
- Shared widget include injected in authenticated pages (dashboard + section pages); persistent bottom-right toggle.
- `/agent` page: conversations list, active thread view, continue buttons, uses same API.
- Small page script collects context:
  - `route` (e.g. `/dashboard`)
  - `country` (UK/IN)
  - lightweight page metadata (card totals, date range if available)
- Widget + /agent page post to backend API:
  - `POST /api/ai/chat`
...
- New service layer:
  ...
  - `lib/ai/prompts.js`
  - `lib/ai/tools.js`
- Tool execution remains server-side only (never client-direct DB access). Tools wrap Postgres reads (spend, income, transactions, category breakdown, accounts).
