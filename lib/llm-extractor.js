...
async function extractTransactionsFromText(statementText) {
  ...
  const prompt = `You are a bank statement parser. Extract all transactions from the following bank statement text and return as JSON. Also suggest a category for each transaction based on the merchant name.
  ...
  "transactions": [
    {
      ...
      "category": "one of: Groceries, Transport, Utilities, Entertainment, Dining, Healthcare, Subscriptions, Salary/Income, Savings, Investments, Insurance, Shopping, Fees, or null if unclear"
    }
  ],
